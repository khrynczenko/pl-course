...
(* helper functions for comparing real numbers since rounding means we should never compare for equality *)
  ...
  case e of
    ...
    | LineSegment(x1, y1, x2, y2) => 
        (case (real_close(x1, x2), real_close(y1, y2)) of
            ...
            | (false, true) => if x1 > x2 then LineSegment(x2, y2, x1, y1) else e
            | (false, false) => if x1 > x2 then LineSegment(x2, y2, x1, y1) else
              e)
    | Var _ => e
    | Let(s,e1,e2) => Let(s, (preprocess_prog e1), (preprocess_prog e2))
    | Intersect(e1, e2) => Intersect((preprocess_prog e1), (preprocess_prog
    e2))
